...
nothing really special, everything is quite straight forward in the code. just check, it is self explanatory.
...
you can retrieve the token from the logs of the backend container.

so basically, the first time, you have to run it twice: once with only the backend container (with docker-compose up -d backend) and another one with all the containers (with docker-compose up -d) *after* putting in your .env the tokens generated by the backend.
